...
#include "mct_pin_di_mbus_cfg.h"
#include "mct_pin_di_mbus_cmd.h"
#include "../if/mbus_if.h"

//#include "mct_pin_di_mbus_test.h"	// TEST-Sequencen

/// MBUS-PLATTFORM DEVICE - Zeiger
static struct platform_device * pdev;
/// MBUS-DEVICE Interface Name
char di_dev_if_mbus[100];

static DECLARE_COMPLETION( mbus_device_is_free );

/// la (typ: m_app) mit Zeiger auf di_O_Mct (typ:tdi_drv)
...
//	.fake_line	= &MBUS_TLGs[0],
	...
	.l2 = {
	.slvs_cfg = MBUS_SLVS,
	.slvs[MBUS_SLV_IN].sec_addr	= {\
		0x00,DRIVER_PIN_DI_IDENT_NUM,0x00,0x00,MBUS_MANU_HEX,MBUS_GEN_HEX,MBUS_MED_HEX},
...
int  mbus_build(struct platform_driver * drv, unsigned char slot) {	
	...
	/// INTERFACE-NAME
	strcpy(di_dev_if_mbus,MBUS_DEVICE_INTERFACE);
	slotstr[0]= slot +0x30;
	strcat(di_dev_if_mbus,slotstr);

	pdev = platform_device_alloc(di_dev_if_mbus,0);
