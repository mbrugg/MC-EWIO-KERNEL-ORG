...
#define DEV_PIN_NAME			"Pulse 2 (S02/2-)"

char *			di_drv_name		= DRV_MCT_PIN_DI_NAME;
char *			di_dev_in_name 	= DEV_MCT_PIN_DI_IN_NAME;
unsigned char 	di_slot			= DRV_MCT_PIN_DI_SLOT;

unsigned char	di_pin			= AT91_PIN_PB7;
char *			di_pin_name		= DEV_PIN_NAME;

///*********
/// INTERCOM
///*********
/// Server setzt im Client 1 das Init-Bit
void icom_svr_set_cli1_init(unsigned char init) {
...
/// Server holt vom Client 1 das Init-Bit
// -1 wenn nicht im S0-Mode, sonst 0 oder 1
int icom_svr_get_cli1_init(void) {
	int init = -1;
	unsigned char mode = 0;
	OBJS_IDEV_RD_MODE(&di_O_Inputs,mode);
	if (mode == MODE_S0) {
...
EXPORT_SYMBOL_GPL( icom_svr_set_cli1_init );
EXPORT_SYMBOL_GPL( icom_svr_get_cli1_init );
