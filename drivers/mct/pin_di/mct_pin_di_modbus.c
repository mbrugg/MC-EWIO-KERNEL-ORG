/*********************************************************************************
	...
	$Date:$		16.01.2012
 	...
 	Description: MODBUS-Interface for mct_pin_di-Driver
...
//	.fake_line = &MODBUS_TLGs[0],
	...
	.slvs[MODBUS_SLV_0].pri_addr	= MODBUS_RTU_TO_INTERN_BASE_ADDR + DRIVER_PIN_DI_IDENT_NUM * 10,
	.cmd_rsp_02						= di_mod_sl2_cmd_rsp_02,	// only Direkt-Mode
	.cmd_rsp_04						= di_mod_sl2_cmd_rsp_04,	// only S0-Mode
	.cmd_rsp_mei_14_code			= di_mod_sl2_cmd_rsp_mei_14_code,
	.pdata							= &di_O_Mct,
	},	
};
...
int  modbus_build(struct platform_driver * drv,unsigned char slot) {	
	...
	la.l2.slvs[MODBUS_SLV_0].pri_addr += slot;
	
	/// INTERFACE-NAME
	strcpy(di_dev_if_modbus,MODBUS_DEVICE_INTERFACE);
	slotstr[0]= slot +0x30;
	strcat(di_dev_if_modbus,slotstr);
	pdev = platform_device_alloc(di_dev_if_modbus,0);
