/*********************************************************************************
	...
	$Date:$		31.08.2011
 	...
 	Description: MBUS-Interface for mct_paa_dio42-Driver
...
#include "mct_dio42_mbus_cmd.h"
#include "mct_dio42_mbus_cfg.h"
#include "../../if/mbus_if.h"

/// MBUS-PLATTFORM DEVICE - Zeiger
static struct platform_device * pdev;
/// MBUS-DEVICE Interface Name
char dio42_dev_if_mbus[100];

static DECLARE_COMPLETION( mbus_device_is_free );

/// la (typ: m_app) mit Zeiger auf dio_O_Mct (typ:tdio42_drv)
...
static struct m_app la = {
	...
	.l2 = {
	.slvs_cfg	= MBUS_SLVS,
	.slvs[MBUS_SLV_IN].sec_addr = {0x00,DRIVER_PAA_DIO42_IDENT_NUM,0x00,0x00,MBUS_MANU_HEX,MBUS_GEN_HEX,MBUS_MED_HEX},
	.slvs[MBUS_SLV_IN].access_cnt = 1,
	.slvs[MBUS_SLV_IN].state = VARIABLE_DATA_STATUS_POWER_LOW,
	.slvs[MBUS_SLV_OUT].sec_addr = {0x00,DRIVER_PAA_DIO42_IDENT_NUM,0x00,0x00,MBUS_MANU_HEX,MBUS_GEN_HEX,MBUS_MED_HEX},
...
int  mbus_build(struct platform_driver * drv,unsigned char slot) {	
	...
	ret= mbus_if_secaddr_by_macaddr(&la.l2.slvs[MBUS_SLV_OUT].sec_addr[1]);	
	la.l2.slvs[MBUS_SLV_OUT].sec_addr[0] = slot *16 + 1;

	/// INTERFACE-NAME
	strcpy(dio42_dev_if_mbus,MBUS_DEVICE_INTERFACE);
	slotstr[0]= slot +0x30;
	strcat(dio42_dev_if_mbus,slotstr);

	pdev = platform_device_alloc(dio42_dev_if_mbus,0);
