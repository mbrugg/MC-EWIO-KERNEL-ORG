/*********************************************************************************
	...
	$Date:$ 	13.10.2011
 	...
 	Description: MODBUS-Command-Implementation for mct_paa_dio42-Driver

 *******************************************************************************/
#include "mct_dio42_objs.h"			// objects
#include "mct_dio42_modbus_cfg.h"		// Konfiguration
...
const char id_code[25] = \
			{1,0,0,3,0,3,'B','T','R',1,8,'M','R','-','D','I','O','4','2',2,4,'V','1','.','0'};

#define CREATE_DRV_PTR 		struct tdio42_drv *drv = (struct tdio42_drv *)(l2->pdata)
#define CREATE_IDEV_PTR		struct tdio42_dev_inp   * idev = drv->Op_Inputs
#define CREATE_ODEV_PTR		struct tdio42_dev_outp  * odev = drv->Op_Outputs
#define CREATE_L1_PTR 		struct mod_l1  *  l1  = &(l2->l1)
#define GET_DEV_INPUT(val)	{	\
	spin_lock_bh(&idev->Op_Value->lock);	\
	val = idev->Op_Value->P_ValueIn.value;	\
	spin_unlock_bh(&idev->Op_Value->lock);}

#define SET_DEV_INPUT(val)	{	\
	spin_lock_bh(&idev->Op_Value->lock);	\
	idev->Op_Value->P_ValueIn.value = val;	\
	spin_unlock_bh(&idev->Op_Value->lock);}
...
/// ------------------------------------------------------------------
int dio42_mod_sl2_cmd_rsp_mei_14_code(struct mod_l2 * l2, unsigned char code)	{
	CREATE_L1_PTR;
	unsigned char i;
	
	MODBUS_CMD_TRACE("dio42_mod_sl2_cmd_rsp_mei_14_code() code: %d\n", code);
...
/// ------------------------------------------------------------------
/// OVERLOAD Function01		Read Coils	 - is READ ONLY
/// ------------------------------------------------------------------
int dio42_mod_sl2_cmd_rsp_01(struct mod_l2 * l2)	{
	CREATE_DRV_PTR;
	...
	MODBUS_CMD_TRACE("dio42_mod_sl2_cmd_rsp_01()\n");
...
/// ------------------------------------------------------------------
int dio42_mod_sl2_cmd_rsp_02(struct mod_l2 * l2)	{
	CREATE_DRV_PTR;
	CREATE_IDEV_PTR;
	CREATE_L1_PTR;
	...
	MODBUS_CMD_TRACE("dio42_mod_sl2_cmd_rsp_02()\n");
...
/// ------------------------------------------------------------------
int dio42_mod_sl2_cmd_rsp_05(struct mod_l2 * l2)	{
	CREATE_DRV_PTR;
	CREATE_ODEV_PTR;
	CREATE_L1_PTR;
	unsigned char n,relay;

	MODBUS_CMD_TRACE("dio42_mod_sl2_cmd_rsp_05()\n");
	...
	if((l1->rec.cnt != 8) ||						/* Wert muss da sein		*/
		...
		l2->exc = EXC_ILLEGAL_DATA_VALUE;
			...
			(l1->rec.buf[3] > 1))	{				/* Adr-Low muss 0..1 sein	*/
		l2->exc = EXC_ILLEGAL_DATA_ADDRESS;
	}
	else {
	/// WERT...
...
/// ------------------------------------------------------------------
int dio42_mod_sl2_cmd_rsp_15(struct mod_l2 * l2)	{
	CREATE_DRV_PTR;
	CREATE_ODEV_PTR;
	CREATE_L1_PTR;
	...
	MODBUS_CMD_TRACE("dio42_mod_sl2_cmd_rsp_15()\n");
	...
	if(	(l1->rec.cnt < 9) ||						/* Byte-Count muss da sein 	*/
		...
		l2->exc = EXC_ILLEGAL_DATA_VALUE;
			...
			(l1->rec.buf[3] > 1) ||					/* Adr-Low muss 0..1 sein 	*/
			(quan + l1->rec.buf[3] > 2))	{		/* Endadr muss passen 		*/
		l2->exc = EXC_ILLEGAL_DATA_ADDRESS;
	}
	else	{
	/// WERT...
...
//		printk("GET OUTPUT: 0x%02x\n",relay);
        ...
        n = l1->rec.buf[5];			// Anzahl Bits
        b = l1->rec.buf[7];			// Bits
		if ((a == 0) && (n == 2))	// schneller Spezialfall
			relay = b & 0x03;
		else {						// langsamer Allgemeinfall
			n = ~(0xFF << n);		// Maske
			b &= n;					// Bits maskiert
