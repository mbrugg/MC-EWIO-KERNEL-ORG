...
#include "mct_di4_modbus_cmd.h"
#include "mct_di4_modbus_cfg.h"
#include "../../if/modbus_if.h"
#include "../../if/modbus_app.h"
//#include "mct_di4_modbus_test.h"	// TEST-Sequencen

/// MODBUS-PLATTFORM DEVICE - Zeiger
static struct platform_device * pdev;
/// MODBUS-DEVICE Interface Name
char di4_dev_if_modbus[100];

static DECLARE_COMPLETION( modbus_device_is_free );

/// la (typ: mod_app) mit Zeiger auf di4_O_Mct (typ:tdi4_drv)
...
//	.fake_line			= &TLGs[0],
	...
	.slvs[MODBUS_SLV_0].pri_addr	= MODBUS_RTU_TO_LBUS_BASE_ADDR + DRIVER_PAA_DI4_IDENT_NUM,
	.cmd_rsp_02						= di4_mod_sl2_cmd_rsp_02,
	.cmd_rsp_mei_14_code			= di4_mod_sl2_cmd_rsp_mei_14_code,
	.pdata							= &di4_O_Mct,
	},	
};
...
int  modbus_build(struct platform_driver * drv,unsigned char slot) {	
	...
	/// INTERFACE-NAME
	strcpy(di4_dev_if_modbus,MODBUS_DEVICE_INTERFACE);
	slotstr[0]= slot +0x30;
	strcat(di4_dev_if_modbus,slotstr);
	pdev = platform_device_alloc(di4_dev_if_modbus,0);
