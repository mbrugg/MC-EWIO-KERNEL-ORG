...
#include "mct_di4_schema.h"		// json-schema
#include "mct_di4.h"
...
static ssize_t drv_json_schema_R(struct device_driver *driver, char *buf)	{	
	int   written= 0;
	written += sprintf(buf,JSON_SCHEMA_PAA_DI4_EN);
	return strlen(buf)+1;
}
...
static ssize_t drv_json_R(struct device_driver *driver, char *buf)	{		
	...
	for (idx=0; idx < DEV_MCT_PAA_DI4_IN_CHAN_MAX-1; idx++)	{
...
static ssize_t drv_version(struct device_driver *driver, char *buf)	{		
	strcpy(buf, DRV_MCT_PAA_DI4_VERSION);
	return strlen(buf)+1;
};
...
static ssize_t dev_input_R(struct device *dev, struct device_attribute *attr, char *buf) {	
	struct tdi4_dev_inp *idev =	dev_get_drvdata(dev);
	u32 value = 0;
...
/// DI4-DRIVER-SYSFS
void di4_sysfs_ini(void) {
};	

void di4_sysfs_drv_add(struct device_driver *drv){
	int ret;
...
void di4_sysfs_drv_del(struct device_driver *drv){
