...
#include "mct_di4_mbus_cmd.h"
#include "mct_di4_mbus_cfg.h"
#include "../../if/mbus_if.h"

/// MBUS-PLATTFORM DEVICE - Zeiger
static struct platform_device * pdev;
/// MBUS-DEVICE Interface Name
char di4_dev_if_mbus[100];

static DECLARE_COMPLETION( mbus_device_is_free );

/// la (typ: m_app) mit Zeiger auf di4_O_Mct (typ:tdi4_drv)
...
static struct m_app la = {
	...
	.l2 = {
	.slvs_cfg	= MBUS_SLVS,
	.slvs[MBUS_SLV_IN].sec_addr = {0x00,DRIVER_PAA_DI4_IDENT_NUM,0x00,0x00,MBUS_MANU_HEX,MBUS_GEN_HEX,MBUS_MED_HEX},
...
int  mbus_build(struct platform_driver * drv, unsigned char slot) {	
	...
	/// INTERFACE-NAME
	strcpy(di4_dev_if_mbus,MBUS_DEVICE_INTERFACE);
	slotstr[0]= slot +0x30;
	strcat(di4_dev_if_mbus,slotstr);

	pdev = platform_device_alloc(di4_dev_if_mbus,0);
