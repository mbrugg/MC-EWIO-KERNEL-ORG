...
#include "mct_ai8_modbus_cmd.h"
#include "mct_ai8_modbus_cfg.h"
#include "../../if/modbus_if.h"
#include "../../if/modbus_app.h"
//#include "mct_ai8_modbus_test.h"	// TEST-Sequencen

/// MODBUS-PLATTFORM DEVICE - Zeiger
static struct platform_device * pdev;
/// MODBUS-DEVICE Interface Name
char ai8_dev_if_modbus[100];

static DECLARE_COMPLETION( modbus_device_is_free );

/// la (typ: mod_app) mit Zeiger auf ai8_O_Mct (typ:tai8_drv)
...
//	.fake_line			= &TLGs[0],
	...
	.slvs[MODBUS_SLV_0].pri_addr	= MODBUS_RTU_TO_LBUS_BASE_ADDR + DRIVER_PAA_AI8_IDENT_NUM,
	.cmd_rsp_04						= ai8_mod_sl2_cmd_rsp_04,
	.cmd_rsp_mei_14_code			= ai8_mod_sl2_cmd_rsp_mei_14_code,
	.pdata							= &ai8_O_Mct,
	},	
};
...
int  modbus_build(struct platform_driver * drv,unsigned char slot) {	
	...
	/// INTERFACE-NAME
	strcpy(ai8_dev_if_modbus,MODBUS_DEVICE_INTERFACE);
	slotstr[0]= slot +0x30;
	strcat(ai8_dev_if_modbus,slotstr);

	pdev = platform_device_alloc(ai8_dev_if_modbus,0);
